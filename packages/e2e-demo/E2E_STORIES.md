...
*(or — if we add cascade — flip the assertion. For v1, assert current behavior.)*
...
*(this exists to document the auth/arbac orthogonality — `@ArbacPublic` does NOT bypass `setupAuthMoost`'s guard.)*
...
*Note:* HOTP-style replay defense isn't standard for TOTP; accept replay-within-window as known. Story documents this.
...
*(documented limitation: anyone with the magic link can accept. Test that step 3 input must include the password — mere visit doesn't activate.)*
...
**Acceptance:** consumer doesn't need to manually decorate `AuthController` / `LoginWorkflow` / `RecoveryWorkflow` / `InviteWorkflow`. Confirm by registering them WITHOUT extra decoration and calling `applyArbacGuardGlobally`.
