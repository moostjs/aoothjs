...
| auth         | `defineRole().allow("auth.invite", "start")`                                                                                                       | enables `auth.invite` workflow start (admin phase-A steps gated via class-level `@ArbacResource('auth.invite') @ArbacAction('start')` on the bundled `InviteWorkflow`) |
...
- `POST /wf/admin` — protected. ARBAC is carried by the bundled `InviteWorkflow` (`@ArbacResource("auth.invite") @ArbacAction("start")` on phase-A steps; phase-B steps are method-level `@Public()` so anonymous magic-link resume isn't denied); allow-list = `["auth.invite"]`.
