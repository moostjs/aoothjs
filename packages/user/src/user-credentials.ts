...
export class UserCredentials<T extends object = { id: string }> extends Changeable {
  ...
  create(
    config: TPasswordConfig & Pick<Required<Required<TAoothConfig>["password"]>, "saltGenerator">,
  ) {
    this.data = {
      id: "",
  ...
  getMfaOptions() {
    ...
    const options = [];
    if (mfa.email.confirmed)
      options.push({
        type: "email",
        value: mfa.email.address,
        masked: this.getEmailMasked(),
        isDefault: mfa.default === "email",
      });
    if (mfa.sms.confirmed)
      options.push({
        type: "sms",
        value: mfa.sms.number,
        masked: this.getPhoneMasked(),
        isDefault: mfa.default === "sms",
      });
    if (mfa.totp.secretKey)
      options.push({
        type: "totp",
        value: mfa.totp.secretKey,
        masked: "",
        isDefault: mfa.default === "totp",
      });
    return options;
  }
  ...
  isLocked(unlockWhenEnded?: boolean) {
    ...
    if (
      ...
      account.lockEnds > 0 &&
      account.locked &&
      account.lockEnds < new Date().getTime()
    ) {
      this.unlockAccount();
    ...
    return {
      ...
      endsIn: account.locked
        ? account.lockEnds > 0
          ? `Block is effective for ${Math.ceil((account.lockEnds - new Date().getTime()) / 1000 / 60)} more minute(s)`
          : ""
        : "",
    };
  }
...
function mask(s: string) {
  if (s) {
    return s.length > 2
      ? s.slice(0, Math.floor(s.length / 4)) + "***" + s.slice(-Math.ceil(s.length / 4))
      : "***";
  }
  return "";
}
