...
| 21  | [ISSUE-25](ISSUES.md#issue-25--credentialstoreencapsulated-lacks-revocation-epoch--fix)                                    | FIX — port BUG-4 epoch-map + guard pattern from JWT store to encapsulated store                                                                                                       | [x] (0eb5aea) |
...
| ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| 2.1  | Define `LoginWorkflowOptions`, `RecoveryWorkflowOptions`, `InviteWorkflowOptions` shells with reasonable defaults (per WF\*.md). Constructor takes `Partial<...>` and `Object.assign`s.                                                                                                                                                      | [x] (13f44e5) |
| 2.2  | Refactor `LoginWorkflow` constructor — DI: opts + `UserService` + `AuthCredential` + `MoostAuthConfig` + `EmailSender` (+ optional `SmsSender` later). Drop `cc.instantiate(...)` from step bodies. Add implicit `init` step writing `this.opts → ctx.opts`. Keep step semantics IDENTICAL to today's behavior — feature expansion is BIG 3. | [x] (13f44e5) |
| 2.3  | Same as 2.2 for `RecoveryWorkflow`.                                                                                                                                                                                                                                                                                                          | [x] (13f44e5) |
| 2.4  | Same as 2.2 for `InviteWorkflow`.                                                                                                                                                                                                                                                                                                            | [x] (13f44e5) |
| 2.5  | Move `prepareUser` hook from `MoostAuthWorkflowConfig` → `InviteWorkflowOptions.prepareUser`. Move `emailToUserId` → `RecoveryWorkflowOptions.emailToUserId`.                                                                                                                                                                                | [x] (13f44e5) |
| 2.6  | DELETE `MoostAuthWorkflowConfig` entirely.                                                                                                                                                                                                                                                                                                   | [x] (13f44e5) |
| 2.7  | DELETE `setupAuthWorkflows` entirely.                                                                                                                                                                                                                                                                                                        | [x] (13f44e5) |
| 2.8  | Update e2e demo `app.ts` to wire workflows via `setProvideRegistry` + `registerControllers` directly (no setup function).                                                                                                                                                                                                                    | [x] (13f44e5) |
| 2.9  | All existing workflow tests must still pass. No behavior change in this BIG — only DI shape.                                                                                                                                                                                                                                                 | [x] (13f44e5) |
...
| Step   | Description                                                                                                                                                                                                                                                    | Status        |
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| 3.1.1  | Expand `LoginWorkflowOptions` to the full shape (per WF_LOGIN.md). All 25+ opts with documented defaults.                                                                                                                                                      | [x] (0858690) |
| 3.1.2  | Define `SmsSender` interface in `@aoothjs/auth` (mirrors `EmailSender`); extend the email/SMS event union with `login.pincode`, `recovery.pincode`, `invite.pincode`, `notifyNewDevice`.                                                                       | [x] (0858690) |
| 3.1.3  | Implement Phase 1 alternate credential paths: `magicLinkRequest`, `magicLinkSend`, `magicLinkVerified`, `passkey` (stub), `ssoCallback` (stub). Wire `magicLink` alt-action on `credentials`.                                                                  | [x] (0858690) |
| 3.1.4  | Implement Phase 2 inline guards (already present today; verify behavior).                                                                                                                                                                                      | [x] (0858690) |
| 3.1.5  | Implement Phase 3 enrollment loops: `ensureEmail` + `ensurePhone` (with `ask-email`/`ask-phone-number` + `set-mfa/<transport>` + `pincode-send-activate` + `pincode-confirm`).                                                                                 | [x] (0858690) |
| 3.1.6  | Implement Phase 4 MFA: `prepare-mfa-options`, `select2fa`, `pincode-send-login`, `pincode-check-login`, `mfa-totp`, `mfa-backup-code`, `mfa-enroll-required`. Define new atscript form models: `Select2faForm`, `PincodeForm`, `AskEmailForm`, `AskPhoneForm`. | [x] (0858690) |
| 3.1.7  | Implement device-trust: `check-trusted-device` (start of Phase 4) + `device-trust` (after MFA) + `DeviceTrustStore` interface (in-memory default; document Redis impl path). HMAC-signed cookie.                                                               | [x] (0858690) |
| 3.1.8  | Implement Phase 5 forced password change: `prepare-password-rules` + `create-password-form`.                                                                                                                                                                   | [x] (0858690) |
| 3.1.9  | Implement Phase 6 acceptance/onboarding: `terms-accept`, `profile-complete`, `consent-marketing`. New form models.                                                                                                                                             | [x] (0858690) |
| 3.1.10 | Implement Phase 7 tenant/persona: `tenant-select`, `persona-select`.                                                                                                                                                                                           | [x] (0858690) |
| 3.1.11 | Implement Phase 8 session policy: `concurrency-limit`, `risk-step-up`.                                                                                                                                                                                         | [x] (0858690) |
| 3.1.12 | Implement Phase 9 finalize: `audit-login`, `notify-new-device`, `redirect`. Existing `issue` step stays.                                                                                                                                                       | [x] (0858690) |
| 3.1.13 | Wire all alt actions per the catalog (every form-bearing step gets `@AltAction()` + routing).                                                                                                                                                                  | [x] (0858690) |
| 3.1.14 | Tests: full coverage matrix per WF_LOGIN.md task #6.                                                                                                                                                                                                           | [x] (0858690) |
| 3.1.15 | e2e demo: pick representative subset (per WF_LOGIN.md task #7).                                                                                                                                                                                                | [x] (0858690) |
...
| Step  | Description                                                                                                                                                                                                                                           | Status        |
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| 3.2.1 | Expand `RecoveryWorkflowOptions` to the full shape.                                                                                                                                                                                                   | [x] (f8e4d69) |
| 3.2.2 | Define `WorkflowRateLimitStore` interface in a shared location (mentioned in WF_RECOVERY task #2 as `RecoveryRateLimitStore` — rename to shared); ship in-memory default.                                                                             | [x] (f8e4d69) |
| 3.2.3 | Implement step catalog: `request` (with rate-limit + URL-param pre-fill from login workflow), `selectMode`, `sendMagicLink`, `sendOtp`, `checkOtp`, `verifyFactor`, `setPassword`, `revokeSessions`, `audit`, `freshLoginFinish` / `autoLoginFinish`. | [x] (f8e4d69) |
| 3.2.4 | New form models: `OtpCodeForm` (with codeLength field), `RecoveryModeSelectForm`, `RecoveryFactorForm`. Reuse `EmailIdentifierForm` and `SetPasswordForm`.                                                                                            | [x] (f8e4d69) |
| 3.2.5 | Wire OTP via `EmailSender` / `SmsSender` (defined in BIG 3.1.2).                                                                                                                                                                                      | [x] (f8e4d69) |
| 3.2.6 | Wire alt actions per the catalog.                                                                                                                                                                                                                     | [x] (f8e4d69) |
| 3.2.7 | Tests: full coverage matrix per WF_RECOVERY.md task #8.                                                                                                                                                                                               | [x] (f8e4d69) |
| 3.2.8 | e2e demo: keep magicLink default + add otp-mode variant.                                                                                                                                                                                              | [x] (f8e4d69) |
...
| Step   | Description                                                                                                                                                                                                                                                                                                       | Status        |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| 3.3.1  | Expand `InviteWorkflowOptions` to the full shape.                                                                                                                                                                                                                                                                 | [x] (b0a6dc7) |
| 3.3.2  | Add `pendingInvitation: boolean` to `UserCredentials` shape (`@aoothjs/user`); ensure `UsersStoreMemory` and `UsersStoreAtscriptDb` (post-ISSUE-20) propagate it.                                                                                                                                                 | [x] (b0a6dc7) |
| 3.3.3  | Implement three workflows: `auth.invite`, `auth.reInvite`, `auth.cancelInvite`, sharing the user-side accept tail via composed schemas.                                                                                                                                                                           | [x] (b0a6dc7) |
| 3.3.4  | Implement admin-side: `assertAdminAuth`, `prepareAvailableRoles`, `selectSendMode`, `adminInviteForm`, `inferRolesStep`, `preCreateUser`, `sendInviteEmail` / `returnShareableLink`.                                                                                                                              | [x] (b0a6dc7) |
| 3.3.5  | Implement user-side accept tail: `checkPendingInvitation` (with hard-delete-cancellation handling → 410), `idempotentRedirect`, `preparePasswordRules`, `createPasswordForm`, `collectProfile`, `applyProfile`, `unsetPendingInvitation`, `activateUser`, `confirmation`, `freshLoginFinish` / `autoLoginFinish`. | [x] (b0a6dc7) |
| 3.3.6  | Implement `cancelInvite` (hard delete).                                                                                                                                                                                                                                                                           | [x] (b0a6dc7) |
| 3.3.7  | Implement `loadPendingUser` (re-invite path).                                                                                                                                                                                                                                                                     | [x] (b0a6dc7) |
| 3.3.8  | Wire `acceptProfileForm` auto-injection mechanism (consumer-supplied schema → step renders it).                                                                                                                                                                                                                   | [x] (b0a6dc7) |
| 3.3.9  | Wire `applyProfile` escape hatch + default deep-merge fallback.                                                                                                                                                                                                                                                   | [x] (b0a6dc7) |
| 3.3.10 | Wire alt actions per the catalog.                                                                                                                                                                                                                                                                                 | [x] (b0a6dc7) |
| 3.3.11 | Wire shared `WorkflowRateLimitStore` (from BIG 3.2.2).                                                                                                                                                                                                                                                            | [x] (b0a6dc7) |
| 3.3.12 | Audit events: `invite.created`, `invite.resent`, `invite.accepted`, `invite.cancelled`.                                                                                                                                                                                                                           | [x] (b0a6dc7) |
| 3.3.13 | Tests: full coverage matrix per WF_INVITE.md task #11.                                                                                                                                                                                                                                                            | [x] (b0a6dc7) |
| 3.3.14 | e2e demo: convert to new shape; demonstrate custom profile form + `applyProfile`.                                                                                                                                                                                                                                 | [x] (b0a6dc7) |
