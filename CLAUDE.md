...
> **Every request 404s instead (GET → SPA HTML 200, POST → connect's `Cannot POST …`, even `/__test/*` and `/auth/*`)? Check WHO owns :3001 before debugging anything** — `lsof -nP -iTCP:3001 -sTCP:LISTEN`. A sibling project's dev server squatting the port is the usual cause: vite silently bumps this demo to 3002+, the boot log still prints `running at http://localhost:3001` (it echoes `buildApp`'s `port` option, not vite's actual port), and every probe/Playwright request hits the foreign SPA — which looks exactly like "moost router matches nothing". Don't kill the other server; boot on an explicit free port and point the suite at it (all three env/flags matter — `PORT` feeds `buildApp`'s self-URL, which the OAuth/OIDC specs' redirect URIs are built from, so OAuth legs bounce to the squatter if it stays 3001):
>
> ```bash
> DEMO_MODE=test SEED=true PORT=3011 pnpm exec vite --port 3011 --strictPort
> BASE_URL=http://localhost:3011 pnpm run test:e2e
> ```
...
- **`UserCredentials` is a TYPE, not a class.** The shape (`{ id, username, version?, password, account, mfa, trustedDevices? }`) lives in `packages/user/src/types.ts`. `UserService` reads/writes plain rows through the store; partial updates are expressed as `UserStoreUpdate { set, inc }` and applied by the store — there is no internal mutation-tracking class. (No base `email`/`phone`/`backupCodes` — secondary login handles are consumer-declared via `@aooth.user.email`/`@aooth.user.phone`, and backup codes are an app-composed column.)
