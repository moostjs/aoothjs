...
export interface AppUser extends AoothUserCredentials {
    // id (PK / @meta.id — the token subject), username (the one base login
    // handle, @db.index.unique), and version are inherited from the base. The
    // base has NO email/phone — to enable login/recovery by a secondary handle,
    // declare your OWN field, index it @db.index.unique, and tag it
    // @aooth.user.email / @aooth.user.phone (see recovery-and-handles.md).
...
| #   | Rule                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0   | **The stable surrogate `id` is the token subject and the key for ALL reads-by-identity + writes** (`getUser`/`update`/`setPassword`/lock/MFA/trusted-device). The ONE handle entry point is `login(handle, …)` → `findByHandle` (`username` first, then the configured secondary handle fields in order — whatever the wiring resolved from `@aooth.user.email`/`@aooth.user.phone`, never `$or`). `findByIdentifier` (`id`→`username`→ those same secondary handles) is for admin/recovery only — never login. `useAuth().getUserId()` returns this `id`. See [recovery-and-handles.md](./recovery-and-handles.md). |
| 1   | **`createUser` mints `id: randomUUID()`** on the base record (the model's `@db.default.uuid` is a fallback for direct inserts only). `extras.id` overrides. `create` throws `ALREADY_EXISTS` on a duplicate `username` **or** any configured handle-field value (mirrors the DB unique index). (`user-service.ts`.)                                                                                                                                                                                                                                                                                                  |
...
| 3   | **No bundled backup-code API.** There is no `generateBackupCodes` / `consumeBackupCode` service method and no `generateBackupCodePlaintext` export, and the base `UserCredentials` has **no** `backupCodes` field — compose recovery codes yourself: declare your own `backupCodes?: string[]` column on your user model, hash with `hashMfaCode`, store via `users.update(...)`, verify with `verifyMfaCode`.                                                                                                                                                                                                       |
...
| 8   | **`@db.patch.strategy 'merge'`** lives in the shipped `AoothUserCredentials` `.as` model on `password`, `account`, `mfa`, `trustedDevices?`, `seenDevices?`. Re-declaring any of these sub-objects in an extending interface without the annotation flips that field to wholesale replace and breaks the partial-update contract.                                                                                                                                                                                                                                                                                    |
...
| 11  | **`deviceTrust.secret` is mandatory for trusted-device AND seen-device APIs.** `issueTrustedDevice` / `verifyTrustedDevice` / `issueSeenDevice` / `verifySeenDevice` throw a plain `Error` (not `UserAuthError`) when the config key is unset; probe with the non-throwing `hasDeviceTrustSecret()`. The trust HMAC binds `userId\|raw\|ip` (verify must pass the same `ip` you issued with); the seen-device HMAC is domain-separated (`seen\|userId\|raw`, never IP-bound) — the two token kinds are never interchangeable.                                                                                        |
...
| 13  | **`account.verifiedEmail` is correspondence-only — NEVER a login handle.** No uniqueness, never used for account resolution; it records inbox proof so security notices reach users without email-MFA. Recipient chain = `getCorrespondenceEmail(user)`: `config.emailField` column → `verifiedEmail` → confirmed email-MFA. See [user-service.md § Correspondence email](./user-service.md#correspondence-email); workflow capture points: [workflows.md](./workflows.md) invariant 19.                                                                                                                             |
...
| `UserService`      | [user-service.md](./user-service.md)     | Every public method (`createUser`, `getUser`, `login`, `verifyPassword`, `changePassword`, `setPassword`, `deleteUser`, `update`, `activate*`, `lock*`, `unlock*`, `getLockStatus`, `checkPolicies`, MFA methods, `verifyTotpSetupCode`, `verifyMfa`, `isPasswordExpired`, trusted-device APIs, seen-device recognition ledger, `setVerifiedEmail` / `getCorrespondenceEmail`), config defaults, login sequence |
