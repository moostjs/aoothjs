...
| #   | Rule                                                                                                                                                                                                                                                                                                              |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- | ---------------------------------------------------- |
| 1   | **`createUser` deliberately omits `id`** so the underlying store / DB default (e.g. `@db.default.uuid`) decides. Callers that want a specific id pass it via `extras`. (`user-service.ts:106`.)                                                                                                                   |
| 2   | **`extras` shallow-merges over the base record after `id` is omitted.** Top-level keys (`account`, `mfa`, `password`) are wholesale replaced when present in `extras` — pass nested objects with every required sub-field if you intend to override.                                                              |
| 3   | **`consumeBackupCode` is not atomic.** Two concurrent consumes of the same code can both succeed (read-then-write at the service layer). Wrap in the store's transaction primitive if strict guarantees are required. (`user-service.ts:389`.)                                                                    |
...
| 5   | **`account.lockEnds: 0` means permanent lock**, not "no lock". Expiration is `lockEnds > 0 && lockEnds < now`. `lockAccount(username, reason)` (no `duration`) produces a permanent lock.                                                                                                                         |
...
| `UserService`      | [user-service.md](./user-service.md)     | Every public method (`createUser`, `getUser`, `login`, `verifyPassword`, `changePassword`, `setPassword`, `deleteUser`, `update`, `activate*`, `lock*`, `unlock*`, `getLockStatus`, `checkPolicies`, MFA methods, `generateBackupCodes`, `consumeBackupCode`, `verifyMfa`, trusted-device APIs), config defaults, login sequence |
...
| MFA                | [mfa.md](./mfa.md)                       | TOTP primitives + constant-time verification + window, otpauth URI for QR, `generateMfaCode` / `hashMfaCode` / `verifyMfaCode` for email/SMS challenges, `generateBackupCodePlaintext` alphabet, trusted-device tokens (HMAC, IP binding, `deviceTrust.secret`)                                                                  |
