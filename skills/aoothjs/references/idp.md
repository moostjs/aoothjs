...
| 7   | **`email_verified` is read STRICTLY as boolean** by `OidcProvider`/`GoogleProvider` — a non-boolean (string `"true"`) yields `emailVerified: undefined`. **`AppleProvider` is the exception** (overrides `normalize` to coerce Apple's spec-violating string `"true"`/`"false"`). **`GithubProvider`** sets `emailVerified:true` ONLY for the verified PRIMARY `/user/emails` entry (non-primary / unverified / scope-absent fallback → `false`).                                                                                                                                                                                                                                                                |
| 13  | **`GithubProvider`** — OAuth2-only (no id_token/JWKS/nonce; `nonce` arg ignored). `exchange` redeems `code` (token endpoint asked for JSON) → `GET /user` + `GET /user/emails`; `subject = String(user.id)`, `displayName = name ?? login`. Sends a `User-Agent` (GitHub rejects requests without one; default `'aoothjs'`, override `userAgent`). Endpoints overridable (Enterprise). GitHub stays OUT of `trustEmailVerifiedFrom` by default. Token failures (200+`{error}`, non-2xx, missing `access_token`) → `EXCHANGE_FAILED`.                                                                                                                                                                             |
| 14  | **`AppleProvider`** — `extends OidcProvider` (issuer `https://appleid.apple.com`, `ES256`). NO static secret: a `clientSecret` **factory** (`ClientSecretFactory`) mints a per-request ES256 `client_secret` JWT from the `.p8` key (`iss=teamId`, `sub=clientId`, `aud=appleid`, header `kid`+`alg ES256`), cached until ~1 min before `exp` (`clientSecretTtlSec`, default 1 h). Declares `response_mode=form_post` (callback is a cross-site POST → auth-moost `OAuthController` POST→GET bounce; see [oauth.md](oauth.md)). First-login `name` (form_post `user` field) NOT captured in v1 → `displayName` undefined. Ctor requires `teamId`/`keyId`/`privateKey`; bad `.p8` → `INVALID_CONFIG` at exchange. |
