...
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `@aoothjs/user`        | `UserService` orchestrator, scrypt `PasswordHasher`, `PasswordPolicy` engine + 6 transferable factories, TOTP/HOTP, backup-code generation/hashing, trusted-device HMAC, lockout, `UserStore` abstract + `UserStoreMemory`               | Email/SMS delivery, JWT/session issuance, role storage, MFA challenge state machine                                       |
...
| "`.as` user model, `@arbac.role` / `.attribute` / `.userId`, atscript plugin"      | this skill + see [moost.md](./moost.md) (annotations.md) |
...
| `@aoothjs/auth`        | `jose`                 | `^6.2.3`          | no (hard dep — `CredentialStoreJwt` uses it)  |
...
| `@aoothjs/arbac-moost` | `./atscript/models/user` | The `AoothArbacUserCredentials` `.as` interface (raw file export)                           |
