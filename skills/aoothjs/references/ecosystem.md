...
| `@aooth/auth-moost`  | `AuthController` (`/auth/{logout,refresh,status,trigger,invite/post-redemption}` + gated `/auth/{change-password,add-mfa}`), `authGuardInterceptor`, `@Public` (dual auth+arbac), `@UserId`, `useAuth`, unified `AuthWorkflow` (login/invite/recovery/signup/change-password/add-mfa), `ConsentStore`, `WfTriggerProvider` | Email/SMS delivery (consumer ships `EmailSender`/`SmsSender`), authorization (handled by `arbac-moost`)                   |
...
`@atscript/moost-db` is a security floor for `@aooth/arbac-moost` (`^0.1.134` from 0.1.68; 0.1.67 peers on `^0.1.133`) — see [column-scope security floor](https://aoothjs.dev/moost/db-controllers#column-scope-security-floor).
