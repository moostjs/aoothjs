...
**Column scopes** need `@atscript/moost-db` ≥ 0.1.133 and `arbacAuthorizeInterceptor` / `@ArbacAuthorize()` on the controller (they fail open without it) — details: [column-scope security floor](https://aoothjs.dev/moost/db-controllers#column-scope-security-floor).
