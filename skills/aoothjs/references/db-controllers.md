...
| Hook                              | When                             | Behavior                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| --------------------------------- | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `transformFilter(filter)`         | Every read query                 | Merges user filter with union of scope filters via `conjoinScopeFilters` → `$and: [merged, userFilter]` — **never object spread**, which would let a user filter on the same field replace the scope's and widen access. On deny returns a match-nothing filter (`{ $or: [] }`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `transformProjection(projection)` | Every read query                 | Unions per-scope `projection` whitelists and narrows the user `$select` to the INTERSECTION with that union — array (inclusion) and object (exclusion) forms both; a requested parent whose descendants the scope constrains is split by schema (`$select=a` under `{ "a.b": 1 }` or `{ "a.c": 0 }` → only `a.b`); excluded nested objects are stripped by their leaves. Same rule inside `$with` sub-selects. Empty union → unrestricted.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `applyMetaOverlay(meta)`          | `/meta` requests                 | Evaluates ARBAC in parallel for every declared action and CRUD op; filters `meta.actions` and `meta.crud`, then PRUNES the field surface (`fields`, serialized `type`, `relations`, `versionColumn`) by the union of the allowed read ops' scope projections (BUG-3) — a scoped UI cannot even offer hidden columns. PK + `preferredId` always survive; unscoped read grant → full envelope; NO read op allowed → field surface untouched (write-only forms need `type`). Relations survive when projected OR `with`-granted; a `with`-granted relation's nav type in `type` is pruned by its sub-scope (recursively), matching `hasField`. Since 0.1.57, fields in the caller's WRITE scopes' `allowedFields` union (or any field under an unrestricted/whitelist-free write grant) that fall OUTSIDE the read projection are STAMPED instead of pruned: they stay in `fields` as `{ writeOnly: true, filterable: false, sortable: false }` and in the serialized `type` with `db.writeOnly` metadata (subtree kept whole) — generic forms can SET sealed fields the caller can never read. Shared impl: `applyArbacMetaOverlay(meta, this.readable)` — identifiers + related tables derive from the readable (a bare identifier set, the 0.1.67 form, skips related-table identifiers). |
| `hasField(path)`                  | Every gated query position       | Scope-aware existence (BUG-3 twin): paths outside the read-projection union answer `false` → any query reference to a hidden field gets the IDENTICAL `Unknown field "x"` 400 a nonexistent field gets (no existence/value oracle). Identifiers always pass. A `rel.x` path under a `with`-granted relation (filter / `$sort` / `$select` inside `$with=rel(...)`) is checked as `x` against the UNION of the `with.rel` sub-scopes, recursively for nested `$with`; the related table's PK / `preferredId` always pass; no `with.rel` in any role → unrestricted (silence wins). Shared body: `isScopedFieldVisible(getArbacScopes() ?? [], path, this.readable)` (`getArbacScopes` = direct slot read, no `useArbac()` metadata resolution per field). Requirements: [column-scope security floor](https://aoothjs.dev/moost/db-controllers#column-scope-security-floor).                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `assertInScope(idOrIds, scopes)`  | Reused by `onWrite` + `onRemove` | Issues a `readable.count` with `{ $and: [resolveIdFilter(id), mergedScopeFilter] }` and throws `HttpError(404, 'Not found')` if not every id is in scope. **Returns 404, not 403, to hide existence.** Ids resolve through moost-db's `hasField` predicate: a unique key the read scope hides is NO identification — DELETE / PATCH / PUT by a hidden existing value answers identically to a nonexistent one (0.1.68+).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
**Column scopes** need `@atscript/moost-db` ≥ 0.1.134 (arbac-moost 0.1.68 peers on `^0.1.134`: hidden unique keys never identify a row; an excluded nested object drops its whole subtree) and `arbacAuthorizeInterceptor` / `@ArbacAuthorize()` on the controller (they fail open without it) — details: [column-scope security floor](https://aoothjs.dev/moost/db-controllers#column-scope-security-floor).
...
**`with[name]`** is a recursive sub-scope applied when the request expands the `name` relation via `?$with=<name>`. The PARENT scope owns the policy for joined rows — arbac-moost does NOT re-evaluate ARBAC against the joined resource. Across roles, `with[name]` sub-scopes union additively at every nested level. A field the sub-scope hides is an unknown field in `$with` sub-queries (`$with=author(salary>1)` → 400 `Unknown field "author.salary"`, identical to a nonexistent one) and is pruned from the `/meta` nav type. To hide a related column, DECLARE `with.<rel>` — silence leaves joined rows and their paths unrestricted. Through 0.1.67 these paths skipped the sub-scope check (a value oracle).
