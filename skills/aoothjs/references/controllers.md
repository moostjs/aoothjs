...
`DEFAULT_AUTH_WORKFLOWS = ["auth/login/flow", "auth/invite/start", "auth/recovery/flow"] as const`. The change-password wfid is exported separately as `CHANGE_PASSWORD_WORKFLOW = "auth/change-password/flow"` — deliberately excluded from the public trigger's allow-list.
...
| `refreshCookie.path` | `'/auth/refresh'`     | **Narrow path** so refresh isn't sent to other endpoints.                                       |
...
| `getUserId()`                       | `string`                       | Throws `HttpError(401, "Not authenticated")` when no context. **Returns the username string** — the auth subject set by `UserService.login(username, password)`. If your user table's `@meta.id` is a different field (e.g. uuid), annotate `username` with `@arbac.userId` (via `annotate AoothUserCredentials { @arbac.userId username }` if inherited) so `AtscriptArbacUserProvider`'s lookup chain points at it. |
