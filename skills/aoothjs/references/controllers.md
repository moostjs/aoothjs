...
REST surface and composables. Covers `AuthController`'s seven endpoints (five `@Public()`, two ARBAC-gated), `authGuardInterceptor` options, `useAuth()` / `useArbac()` API, the decorator quartet, and the 401-vs-403 split. Workflow internals live in [workflows.md](workflows.md); DB scoping in [db-controllers.md](db-controllers.md).
...
Class-decorated `@Controller("auth") @ArbacResource("auth")`. Constructor `(auth: AuthCredential, @Optional() users?: UserService)` — `users` is optional; only `GET /auth/invite/post-redemption` reads it (returns 500 when unset). Five of the seven routes are `@Public()`; auth is enforced **inside** those handler bodies (defence-in-depth) so anonymous callers can still reach login / refresh / invite-redeem. The exceptions are `POST /auth/change-password` and `POST /auth/add-mfa` — both ARBAC-gated authenticated self-service flows (NOT `@Public()`, gated by `@ArbacResource(...) @ArbacAction("self")`) — see their rows.
...
| POST   | `/auth/change-password`        | `@ArbacResource("auth.change-password") @ArbacAction("self") @WfTrigger({ allow: [CHANGE_PASSWORD_WORKFLOW] })` | `{ wfs?, input?: { action?, formData? } }` | `WfFinished` envelope         | GUARDED trigger for `auth/change-password/flow` (NOT `@Public()`, NOT in `DEFAULT_AUTH_WORKFLOWS`). 401 if unauthenticated; 403 unless the principal's role grants `allow("auth.change-password","*")`. Method-level `@ArbacResource` overrides the class `"auth"`. See [workflows.md § change-password](workflows.md#invariants).                                                                                                                               |
| POST   | `/auth/add-mfa`                | `@ArbacResource("auth.add-mfa") @ArbacAction("self") @WfTrigger({ allow: [ADD_MFA_WORKFLOW] })`                 | `{ wfs?, input?: { action?, formData? } }` | `WfFinished` envelope         | GUARDED trigger for `auth/add-mfa/flow` — the profile-maintenance twin of change-password (authenticated user adds a 2nd factor). NOT `@Public()`, NOT in `DEFAULT_AUTH_WORKFLOWS`. 401/403 as above, keyed on `allow("auth.add-mfa","*")`. Reuses login's enroll trio; `init-add-mfa` narrows `availableTransports` to un-enrolled methods (empty ⇒ `{ added: false, reason: "no-methods-available" }`). See [workflows.md § add-mfa](workflows.md#invariants). |

`DEFAULT_AUTH_WORKFLOWS = ["auth/login/flow", "auth/invite/start", "auth/recovery/flow", "auth/signup/flow"] as const`. The two gated wfids are exported separately and deliberately excluded from the public trigger's allow-list: `CHANGE_PASSWORD_WORKFLOW = "auth/change-password/flow"` and `ADD_MFA_WORKFLOW = "auth/add-mfa/flow"` — each is the sole entry on its own route's `@WfTrigger({ allow })`. Granting the matching ARBAC resource (`allow("auth.change-password","*")` / `allow("auth.add-mfa","*")`) is the on/off switch. (`auth/signup/flow` IS public but gated by `resolveSignupPolicy().allowSignup`, default off.)
...
| ----------------------------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `getAuthContext<TPayload>()`        | `AuthContext<TPayload> \| null`               | The stashed context.                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
