...
| `@db.json`                          | Consumer-declared structured columns (the `@aooth.auth.metadata` column, an `@arbac.attenuate.role string[]` field, …) | Structured (object/array) columns. SQL adapters store as a JSON column; mongo nests natively. The base credential has NO free-form `claims` blob and NO `metadata` column — per-token data is typed root columns a consumer adds when they `extends AoothAuthCredential`; metadata is the consumer's `@aooth.auth.metadata`-tagged `@db.json` field, shaped `AoothCredentialMetadataBase & { ...extension }` (exported from `@aooth/auth/atscript-db/model` — see auth-stores.md). |
| `@db.default.uuid`                  | `AoothUserCredentials.id` (shipped on the base)                                                                        | The base model declares `@meta.id @db.default.uuid id`. `UserService.createUser` mints `id: randomUUID()` itself, so this default only fires for direct inserts that bypass the service. A consumer can still override via `extras.id`.                                                                                                                                                                                                                                            |
...
| `@ui.form.pushDown`     | `atscript-ui-forms` | Renders a `ui.action` field BELOW the submit button (`pushDownFields`) instead of inline above. Bundled cross-flow links use it: login `signup`/`magicLink`, signup `backToLogin`, recovery `backToLogin`.                                                                                                                                          |
| `@ui.form.attr`         | `atscript-ui-forms` | Static name/value forwarded to the field component. On a `ui.action` field (the `AsAction` button): `'text'` = leading text → `text [link]`; `'align'` = `left`/`center`/`right`. Cross-flow links use `text` + `align` `center`. NOT for the inline-on-input action variant (e.g. `forgotPassword`). See [spa-components.md](./spa-components.md). |
