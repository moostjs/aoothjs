...
| `@db.json`                          | `AoothAuthCredential.metadata` (+ a consumer's structured payload columns, e.g. an `@arbac.attenuate.role string[]` field) | Structured (object/array) columns. SQL adapters store as a JSON column; mongo nests natively. The base credential has NO free-form `claims` blob — per-token data is typed root columns a consumer adds when they `extends AoothAuthCredential`.                                            |
