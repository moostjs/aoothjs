...
ONE class `AuthWorkflow` with five `@Workflow` methods. Replaces the removed `LoginWorkflow` / `RecoveryWorkflow` / `InviteWorkflow` quartet. Login/invite/recovery/signup dispatch via `POST /auth/trigger` (all in `DEFAULT_AUTH_WORKFLOWS`); change-password via its own guarded `POST /auth/change-password`.
...
| 11  | **`@Public()` placement:** the login/invite/recovery/signup `@Workflow` bodies are `@Public()`; reachable-anon `@Step`s are per-step `@Public()`; invite admin-phase steps omit it (ARBAC-gated on the `invite` permission). **change-password is fully gated — NO `@Public()` anywhere.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
