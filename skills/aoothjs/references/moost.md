...
| 11  | **Refresh cookie path is narrow + auto-derived** to `AuthController`'s actual `refresh` route (follows mount prefix; override via `refreshCookie.path`). Browsers won't send it elsewhere — `/auth/logout` revokes the current session family by `sessionId`, with `{ refreshToken? }` body as fallback.                                                                                                                                                                                                                           |
...
| 14  | **DB scope+user filter merge is `$and: [scope, user]`, never spread.** A user filter constraining the SAME field as the scope would replace it under a spread (scoped to `tenantId:'a'`, asks for `'b'`, gets tenant b) — `$and` intersects so the contradiction matches nothing. Use `conjoinScopeFilters()` from `@aooth/arbac`. (Originally BUG-2, blamed on `@uniqu/core`'s `walkFilter` short-circuit; that was fixed in uniqu 0.1.8 — the rule still stands for the same-key reason, do NOT "simplify" it back to a spread.) |
...
| Workflows            | [workflows.md](./workflows.md)           | unified `AuthWorkflow` (6 schemas: login/invite/recovery/signup/change-password/add-mfa), `AuthWorkflowOpts` vs `resolveXxx` policy, `ConsentStore`, `WfTriggerProvider` + `createAuthEmailOutlet`, `deliver`, error posture, forms                                           |
| Federated login      | [oauth.md](./oauth.md)                   | "Sign in with Google": login-form SSO button + `beginSso`, `OAuthController` (identities/link/unlink), federated leg of `auth/login/flow` (`sso-callback` + `needs-link` `prove-control`), stateless PKCE, connected accounts, callback bridge, CSRF/redirect/gate invariants |
