...
| Federated login      | [oauth.md](./oauth.md)                   | "Sign in with Google": `OAuthController` (start/link/unlink), `auth/oauth/flow` + `oauth-exchange`, `OAuthFlowStore` + the STRING DI tokens, callback bridge, CSRF/PKCE/redirect/gate invariants |
