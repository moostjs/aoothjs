...
| 4   | **`redirect_uri` (`baseUrl` + `/auth/oauth/:provider/callback`) is YOUR route**, not a backend one (for GET-callback providers). In a SPA it reads `code`/`state` and POSTs them into `/auth/trigger` as the START `input.formData` of `auth/login/flow`. `<AsWfForm :input>` wraps the object as `input.formData`, so pass `{ code, state }` (NOT nested `{ formData }`). Register the same `:components` map as the login page (a federated login can land on MFA/consent/prove-control forms).                                                                                                                                                             |
| 13b | **`form_post` providers (Apple) → server `POST :provider/callback` bounce.** Apple POSTs the callback cross-site (`response_mode=form_post`), which a static SPA page can't read. The `OAuthController` adds `POST /auth/oauth/:provider/callback` that 303-redirects {code,state,error} to the SAME GET SPA callback URL — byte-identical downstream. DUMB adapter (verifies nothing; 404s unknown provider; echoes ONLY code/state/error). POST route + SPA GET route share the path, never collide. Lax CSRF cookie isn't sent on the cross-site POST (not read here) — it rides the later same-origin `/auth/trigger` XHR where `sso-callback` checks it. |
