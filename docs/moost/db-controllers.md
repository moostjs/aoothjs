...
| `hasField(path)`                  | Field-existence parity: paths outside the read-scope projection union answer `false`, so `$select` / filter / sort references to a hidden field get the **identical** `Unknown field "x"` 400 a nonexistent field gets — no existence/value oracle.                                                                                                                   |
