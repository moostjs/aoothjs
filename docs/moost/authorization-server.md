...
- **One-time use with rotation** (OAuth 2.1 §4.3.1): every redemption returns a fresh access + refresh pair; the family's lifetime is a **fixed ceiling** (the policy's `refresh.ttl` from the original grant — rotation never extends it). A grace window (default 30 s; the policy's `graceMs` overrides, `0` = strict) tolerates a benign re-presentation of the just-rotated token by **idempotently re-delivering the same pair** — so a connector whose rotation response died with a redeploying task retries and recovers instead of bricking until re-consent. Size `graceMs` to your redeploy window (30–60 s). **Replay beyond the window — or of a superseded token — revokes the whole family** — the standard theft response, riding `AuthCredential`'s existing family revocation (see [the grace window](../auth/refresh#the-grace-window-and-concurrency)).
