...
- **No HTTP.** No controllers, no decorators, no middleware. Wire it under `@aoothjs/auth-moost` (or your own framework code) for that.
- **No sessions or tokens.** Issuing JWTs, refresh rotation, magic links — those live in `@aoothjs/auth`.
- **No MFA delivery.** `MfaMethod.value` is a string the service stores; sending the SMS / email is your job.
- **No challenge state machine.** The "issue OTP → wait → verify" loop is owned by `@aoothjs/auth` and workflow controllers in `@aoothjs/auth-moost`.
- **No RBAC.** Roles, privileges, data scopes — see [`@aoothjs/arbac`](../arbac/).
...
```ts
import { UserService, UserStoreMemory, ppHasMinLength, ppHasNumber } from "@aoothjs/user";
...
- [`Stores`](./stores) — the `UserStore<T>` contract, `UserStoreMemory<T>` for tests, the `UserStoreUpdate { set, inc }` shape, and the `@atscript/db` adapter (`@aoothjs/user/atscript-db`).
