...
`@aoothjs/user` ships **primitives**, not a delivery system. This page documents what's in the package — TOTP secret/URI/code/verify, one-time MFA-code helpers, backup codes, trusted devices — and what's deliberately out of scope.

## What's here vs. what's not

| Capability                                                                 | Where it lives                                    |
| -------------------------------------------------------------------------- | ------------------------------------------------- |
| TOTP secret generation, otpauth URI, code generation, constant-time verify | `@aoothjs/user/mfa/totp`                          |
| SHA-256 one-time-code hash + verify (for email/SMS challenges)             | `@aoothjs/user/mfa/codes`                         |
| Backup-code plaintext generation (alphabet, formatting)                    | `@aoothjs/user/mfa/backup-codes`                  |
| `UserService.verifyMfa` (TOTP) + lockout-counter sharing                   | `@aoothjs/user/user-service`                      |
| Trusted-device HMAC tokens + IP binding                                    | `@aoothjs/user/user-service`                      |
| **Email / SMS delivery**                                                   | Your transport layer (or `@aoothjs/auth` senders) |
| **Challenge state machine** ("issue OTP → wait → verify within N min")     | `@aoothjs/auth` + `@aoothjs/auth-moost` workflows |
...
```ts
import { generateTotpUri } from "@aoothjs/user";
...
```ts
import { generateTotpCode, verifyTotpCode } from "@aoothjs/user";
...
This package does **not** persist the issued challenge or its TTL. Use `@aoothjs/auth` (or your own short-lived KV) for the challenge state machine. The primitives here are pure.
:::

## Backup codes

```ts
import { generateBackupCodePlaintext } from "@aoothjs/user";
...
```ts
import { maskEmail, maskPhone, maskMfaValue } from "@aoothjs/user";
...
- `@aoothjs/auth` — challenge state machine, email/SMS senders.
